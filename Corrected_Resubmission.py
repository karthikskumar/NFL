...
myConstr[constrName]=myModel.addConstr(quicksum(myGames[a,h,s,1]for h in T for a in H[h] for s in ['MON1_ESPN','MON2_ESPN']) == 1,name=constrName)
myModel.update()
...
constrName='%s_slot_w%s' %('Late_Mon,1)
myConstr[constrName]=myModel.addConstr(quicksum(myGames[a,h,'MON2_ESPN',1] for h in WC for a in H[h]) == 1, name=constrName)
myModel.update()
...
for w in range(1,15):
    for a in A:
       ...
       myConstr[constrName]=myModel.addConstr(quicksum(myGames[a,h,s,w1] + (myGames[a,'BYE',s,w1] if ((a,'BYE',s,w1) in myGames))) for h in A[a] for w1 in range(w, w+4) for s in S[w1]) <= 3, name = constrName)
myModel.update()
...
for w in range(1,4):
    for a in A:
        ...
        myConstr[constrName]=myModel.addConstr(quicksum(myGames[a,h,s,w1] for h in A[a] for w1 in range(w, w+3) for s in S[w1]) +
                                               quicksum(myGames['BYE',h,s,w1] for h in A[a] for w1 in range(w, w+3) for s in S[w1])<= 2, name = constrName) 
myModel.update()
